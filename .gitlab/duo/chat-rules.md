...
[AGENTS.md](../../AGENTS.md). Command matrix:
...
- **Keep `infracost` commands free of shell metacharacters** (`;`, `&&`, `|`, `$`, `>` redirects).
  ...
  piping to `jq`/`awk`/`cut` or redirecting to files. Metacharacters disable Duo's pattern-based
  ("approve all uses of this tool") approval and fall back to per-session approval. **Only exception:**
  `infracost price` reads IaC from stdin via a heredoc.
