...
Assists when creating or modifying infrastructure as code (IaC) to ensure best practices are followed and costs are optimized from the start. Uses Infracost to analyze code as it's being written, providing real-time feedback on cost implications and optimization recommendations.
...
This skill uses Infracost **capabilities** (list policies / guardrails / budgets, price, scan, and
the inspect drill-ins). Each has two bindings — a typed MCP tool call and an `infracost …` CLI
command — listed in [BINDINGS.md](../../BINDINGS.md).
...
Five capabilities matter for this workflow:

1. **list policies** — all FinOps + tagging policies for the user's organization, so we can write compliant code from the start.
2. **list guardrails** — cost guardrails (spending thresholds) configured for the repository, so we can keep the generated infrastructure within budget.
3. **list budgets** — tag-scoped budgets for the organization, so we can see which tag groups are already near or over their actual cloud spend limits.
4. **price** — price a small, standalone piece of Terraform without committing it to the repo. Use for fast feedback while iterating on individual resources.
...
Before generating any code, use the three "what's the org configured?" capabilities so the generated infrastructure is compliant by construction:
...
- **list guardrails** for the repo path — note the thresholds (`total_threshold` / `increase_threshold` / `increase_percent_threshold`) and the actions (`pr_comment` / `block_pr`). A `block_pr` action means the threshold is a hard constraint — the generated infrastructure must stay under it. Soft constraints (alert / PR comment only) become warnings to surface.
- **list budgets** — every tag-scoped budget. If the resources you're about to create would match an existing budget's tag scope (`env=production`, `team=frontend`, etc.), note where that budget currently sits versus its `amount`. Budgets reflect **actual org-wide cloud billing**, not the preview — treat them as context for the user, not a hard constraint on the code.

Narrow the policies call when you know the provider (e.g. providers = `aws`) to skip downloading provider plugins for clouds you're not touching.

### 2. Iterate quickly with price

While drafting resources, use the **price** capability with a small Terraform snippet to get a cost on what you've written so far. This is faster than writing the whole file and scanning — useful for choosing between two configurations:
...
For non-USD pricing pass a `currency` (any ISO 4217 code).
...
When the summary flags an issue, the inspect capabilities surface the detail. Each reads the latest cached scan automatically; target a specific previously-scanned directory by passing its path.

| Question | Capability |
|---|---|
| Everything failing right now (policies + guardrails + budgets in one view) | *everything currently failing* |
| Top N savings opportunities | *top N savings* (default `n=10`) |
| Resources missing / with-invalid a specific tag | *list resources by predicate* with `missing_tag` or `invalid_tag` |
| Resources by cost band | *list resources by predicate* with `min_cost` / `max_cost` |
| Group resources by type / provider / file | *aggregate resources by dimension* (e.g. `group_by=type`) |
| Per-project parse / lint diagnostics | *per-project diagnostics* (defaults to all severities; `critical_only` to filter) |
| Which resources fail a specific policy | *policy detail* (e.g. policy = "Use GP3") |
| Per-resource detail under a policy | *policy detail* (policy + resource, e.g. "Use GP3" + "aws_ebs_volume.data") |
| Detail for one budget (matching resources + savings) | *budget detail* (e.g. budget = "Production budget") |
| Triggered state + monthly cost for one guardrail | *guardrail detail* (e.g. guardrail = "Cost increase > $100") |
...
- Do not write `jq` pipelines, `python3 -c` heredocs, or shell post-processing to mangle output — Infracost returns structured data already (typed JSON via MCP; `--llm` via the CLI). If the data you need isn't on one of the typed shapes, that's a signal we should add it; file an issue describing the gap.
- Do not stash or affect the target repository's git state — Infracost's read operations are non-destructive and read-only. If you need to compare branches, use separate worktrees away from the user's working directory.
