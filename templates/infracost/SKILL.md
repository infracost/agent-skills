...
```bash
{{infracost-path:infracost}} login
```
...
{{infracost-path:infracost}} analyze /path/to/cloudformation.yaml

# Terraform project directory
{{infracost-path:infracost}} analyze /path/to/terraform/

# Repository root (auto-discovers all IaC projects in nested directories)
{{infracost-path:infracost}} analyze /path/to/repo
```
...
```bash
{{infracost-path:infracost}} analyze /path/to/repo > /tmp/infracost-output.json
```
...
{{infracost-path:infracost}} inspect --file /tmp/infracost-output.json [flags]
```
...
{{infracost-path:infracost}} analyze /path/to/repo > /tmp/infracost-current.json
{{infracost-path:infracost}} analyze /tmp/infracost-baseline/path/to/repo > /tmp/infracost-baseline.json
